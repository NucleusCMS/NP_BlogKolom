...
//		if($category == '' || !empty($catid)) $category = $blog->getCategoryName($catid);
	...
	function readLogAmount($template, $amountEntries, $extraQuery, $highlight, $comments, $dateheads, $category, $offset = 0, $startpos = 0, $cols)
    {
        global $blogName,$manager, $blog, $CONF;
        if ($blogName) {
        ...
        if ($amountEntries > 0) { // $offset zou moeten worden:
	...
	public function showUsingQuery($templateName, $query, $highlight = '', $comments = 0, $dateheads = 1, $cols)
    {
        ...
        $template = & $manager->getTemplate($templateName); // create parser object & action handler
        ...
        $actions->setShowComments($comments); // execute query
        $items = sql_query($query); // loop over all items
        $old_date = 0;
        $rowCount = intval(0);
        echo "<table class=\"blogkolom\">";
        $parser->parse($template['ITEM_HEADER']); // loop over all items
        $itemcount = 0;
        while ($item = sql_fetch_object($items)) {
            ...
            $remaincells--;
            echo "<td>"; // add date header if needed
            if ($dateheads) {
                $new_date = date('dFY',$item->timestamp);
                if ($new_date != $old_date) { // unless this is the first time, write date footer
                    $timestamp = $item->timestamp;
                    if ($old_date != 0) {
                        $oldTS = strtotime($old_date);
                        $param = array('blog'		=> &$b, 'timestamp'	=> $oldTS);
                    ...
                    $param = array('blog'		=> &$b, 'timestamp' => $timestamp);
        ...
        if ($numrows > 0 && $dateheads) {
            $param = array('blog'		=> &$b, 'timestamp'	=> strtotime($old_date));
            $manager->notify('PreDateFoot',$param);
    ...
    function getTotal()
    {
        ...
        if ($query) {
            $this->total=(int)quickQuery($blog->getSqlSearch($query, $amount, '','count'));
            return $this->total;
        }

        $scid=isset($subcatid)?(int)$subcatid:0;
        if ($scid && $this->getOption('multicat') === 'yes') {
            $this->total=(int)quickQuery(
                preg_replace(
                    '/^([\s]*)SELECT[\s]([^\'"=]*)[\s]FROM[\s]/i',
                    sprintf(
                        'SELECT COUNT(*) as result FROM %s as p, ',
                        sql_table('plug_multiple_categories')
                    ),
                    $blog->getSqlBlog(
                        sprintf(
                            'AND i.inumber=p.item_id AND p.subcategories REGEXP "(^|,)%d(,|$)" ',
                            (int)$scid
                        ),
                        'count'
                    )
        ...
        if ($catid && $this->getOption('multicat') === 'yes') {
            $sqlquery = $blog->getSqlBlog(
                sprintf(
                    'AND i.inumber=p.item_id AND p.categories REGEXP "(^|,)%d(,|$)" AND NOT i.icat=%d ',
                    (int)$catid,
                    (int)$catid
                ),
                'count'
            ...
            $sqlquery = preg_replace(
                ...
                $blog->getSqlBlog('AND i.icat='.(int)$catid.' ','count')
            );
            $this->total+=(int)quickQuery($sqlquery);
            return $this->total;
        }
        ...
        $this->total=(int)quickQuery($blog->getSqlBlog($case,'count'));
        return $this->total;
    }
